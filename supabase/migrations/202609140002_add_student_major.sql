-- Day 2, Activity 5: undergraduate major. Run after the Steve occupation migration.
...
select id, 'student-major', false, false
...
begin
  ...
  expected_count := case
    ...
    when p_activity_key = 'steve-occupation' then 1
    when p_activity_key = 'student-major' then 1
  ...
  if p_activity_key = 'steve-occupation' and not exists (
    select 1
    from jsonb_to_recordset(p_responses) as response(prompt_key text, choice text)
    where response.prompt_key = 'steve-occupation-choice'
      and response.choice in ('Farmer', 'Librarian')
  ) then
    raise exception using errcode = 'P0001', message = 'INVALID_RESPONSES';
  end if;
  ...
  if p_activity_key = 'student-major' and not exists (
    select 1
    from jsonb_to_recordset(p_responses) as response(prompt_key text, choice text)
    where response.prompt_key = 'student-major-choice'
      and response.choice in ('Chinese studies', 'Economics')
  ) then
    raise exception using errcode = 'P0001', message = 'INVALID_RESPONSES';
  end if;
